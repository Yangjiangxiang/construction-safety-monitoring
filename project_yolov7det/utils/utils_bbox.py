...
import warnings
# 忽略特定类型的警告
warnings.filterwarnings("ignore", message="The torch.cuda.*DtypeTensor constructors are no longer recommended")
...
class DecodeBox():
    ...
    def decode_box(self, inputs):
        ...
        for i, input in enumerate(inputs):
            ...
            #   batch_size,3,20,20
            #----------------------------------------------------------#
            anchor_w = torch.tensor(scaled_anchors, dtype=torch.float, device='cuda').index_select(1, torch.tensor([0], dtype=torch.long, device='cuda'))
            anchor_h = torch.tensor(scaled_anchors, dtype=torch.float, device='cuda').index_select(1, torch.tensor([1], dtype=torch.long, device='cuda'))
