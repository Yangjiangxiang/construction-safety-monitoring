...
from project_yolov7det.nets.yolo import YoloBody
from project_yolov7det.utils.utils import (cvtColor, get_anchors, get_classes, preprocess_input,
                         resize_image, show_config)
from project_yolov7det.utils.utils_bbox import DecodeBox, DecodeBoxNP
...
class YOLO(object):
    _defaults = {
        "model_path"        : r'weights\pth\best_epoch_weights.pth',
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            draw.rectangle([tuple(text_origin), tuple(text_origin + (label_size[2], label_size[3]))], fill=self.colors[c])
...
class YOLO_ONNX(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "onnx_path"         : 'project_yolov7_pth/model_data/models.onnx',
        "classes_path"      : 'project_yolov7_pth/model_data/coco_classes.txt',
        #---------------------------------------------------------------------#
        #   anchors_path代表先验框对应的txt文件，一般不修改。
        #   anchors_mask用于帮助代码找到对应的先验框，一般不修改。
        #---------------------------------------------------------------------#
        "anchors_path"      : 'project_yolov7_pth/model_data/yolo_anchors.txt',
        "anchors_mask"      : [[6, 7, 8], [3, 4, 5], [0, 1, 2]],
        #---------------------------------------------------------------------#
        #   输入图片的大小，必须为32的倍数。
        #---------------------------------------------------------------------#
...
class yolov7Pth_infer():

    def __init__(self, model_path=r'weights\pth\best_epoch_weights.pth', classes_path='model_data/voc_classes.txt', anchors_path='model_data/yolo_anchors.txt', anchors_mask=[[6, 7, 8], [3, 4, 5], [0, 1, 2]], input_shape=[640, 640], phi='x', confidence=0.5, nms_iou=0.3, letterbox_image=False, cuda=True, onnx_is=False):
        if onnx_is:
            # onnx: VERY slow
            if not model_path.endswith('.onnx'):
                ...
                self.convert_to_onnx(onnx_save_path)
                # print(onnx_save_path)
                self.yolo = YOLO_ONNX(onnx_path=onnx_save_path, classes_path=classes_path, anchors_path=anchors_path, anchors_mask=anchors_mask, input_shape=input_shape, phi=phi, 
                confidence=confidence, nms_iou=nms_iou, letterbox_image=letterbox_image)
            else:
                self.yolo = YOLO_ONNX(model_path=model_path, classes_path=classes_path, anchors_path=anchors_path, anchors_mask=anchors_mask, input_shape=input_shape, phi=phi, confidence=confidence, nms_iou=nms_iou, letterbox_image=letterbox_image)

        else:
            self.yolo = YOLO(model_path=model_path, classes_path=classes_path, anchors_path=anchors_path, anchors_mask=anchors_mask, input_shape=input_shape, phi=phi, confidence=confidence, nms_iou=nms_iou, letterbox_image=letterbox_image)
    ...
    def infer(self, im0):
        draw, preds = self.yolo.detect_image(self.cv2_to_pil(im0))
        return self.pil_to_cv2(draw), im0, preds
    ...
    def convert_to_onnx(self, save_path, simplify=True):
            import onnx
            self.generate(onnx=True)
            ...
            print('Onnx model save as {}'.format(save_path))